...
export default function ZonePrayerTimes() {
	...
	// We'll calculate the theme derived from data later to avoid flicker
	...
	// --- LOGIC: Theme Detection (Day vs Night) ---
	const getTheme = () => {
		try {
			const syuruk = parseTime(data.data[0].syuruk);
			const maghrib = parseTime(data.data[0].maghrib);

			// If current time is between Syuruk and Maghrib, it's Day.
			const isDay = timeNow >= syuruk && timeNow < maghrib;

			if (isDay) {
				return {
					gradient: "from-emerald-700 to-teal-900", // Day Gradient
				...
				return {
					gradient: "from-slate-900 via-indigo-950 to-slate-900", // Night Gradient
			...
			// Fallback to Day theme if error
			return {
	...
	const calculateProgress = () => {
		try {
			...
			let nextTime = parseTime(data.data[0][nextName]);
			let prevTime = parseTime(data.data[0][prevName]);
			const now = new Date();
	...
	return (
		<>
			...
			<main className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 font-sans transition-colors duration-1000">
				<div className="w-full max-w-4xl bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-800">
					{/* Dynamic Hero Section */}
					<section
						className={`relative bg-gradient-to-br ${theme.gradient} p-8 md:p-12 text-center text-white pb-16 transition-all duration-1000 overflow-hidden`}
					>
